...
var iconSizeMap = {
  ...
  '192x192': { density: 'xxxhdpi', type: 'launcher' },
...
function Platform (packageName, platforms) {
  ...
  self.create = function (w3cManifestInfo, rootDir, options, href, callback) {
    ...
    return fileTools.mkdirp(platformDir)
      ...
      // update app's name,url and domain in strings.xml file
      .then(function () {
        self.debug('Updating app\'s name, url and domain elements...');
        return fileTools.readFile(stringsFile, 'utf-8').then(function (xml) {
          var doc = new DOMParser().parseFromString(xml);
          var elements = doc.getElementsByTagName('string');
          for (var i = 0; i < elements.length; i++) {
            if (elements[i].attributes[0].value === 'app_name' 
            ...
            if (elements[i].attributes[0].value === 'app_url'
            && (w3cManifestInfo.content.start_url || w3cManifestInfo.content.mjs_extended_scope[0])) {
              elements[i].textContent = w3cManifestInfo.content.start_url || w3cManifestInfo.content.mjs_extended_scope[0];
            }
            if (elements[i].attributes[0].value === 'start_url'
            && (w3cManifestInfo.content.start_url || w3cManifestInfo.content.mjs_extended_scope[0])) {
              elements[i].textContent = w3cManifestInfo.content.start_url || w3cManifestInfo.content.mjs_extended_scope[0];
            }
            if (elements[i].attributes[0].value === 'app_host'
            && (w3cManifestInfo.content.start_url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "")  
            || w3cManifestInfo.content.mjs_extended_scope[0].replace(/^(?:https?:\/\/)?(?:www\.)?/i, "")))  {
              elements[i].textContent = w3cManifestInfo.content.start_url.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "") 
              || w3cManifestInfo.content.mjs_extended_scope[0].replace(/^(?:https?:\/\/)?(?:www\.)?/i, "");
            }
          }
      ...
      .then(function () {        
        ...
        var manifestDir = path.join(platformDir, 'app', 'src', 'main', 'assets');
